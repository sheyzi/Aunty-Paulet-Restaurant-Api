...
from app.models import (
    ...
    admin_push_token,
    StoreSettings,
    store_settings_pydantic,
    store_settings_pydanticIn,
)
...
@router.get(
    "/orders/pending", response_model=List[order_pydantic]
)
...
async def set_order_status_to_delivered(
    ...
    push_token = await UserPushToken.get(
        user_id=order.user_id
    )
    await send_push_notification(
        [push_token.push_token],
        "Order delivered",
        "Your order of ₦{:.2f} has been delivered".format(
            order.amount
        ),
        {"screen": "Profile"},
    )
...
async def get_user_token(
    ...
    push_obj = await AdminPushToken.get_or_none(
...
async def add_admin_push_token(
    token: str,
    current_user: user_pydanticOut = Depends(
        get_admin_user
    ),
    ...
    if push_obj != None:
        await AdminPushToken.filter(id=push_obj.id).update(
            push_token=token
        )
        return {
    ...
    push_obj = await AdminPushToken.create(
...
@router.get(
    "/categories", response_model=List[category_pydantic]
)
...
@router.get(
    "/products", response_model=List[product_pydantic]
)
...
@router.post(
    "/categories/add", response_model=category_pydantic
)
async def add_category(
    request: category_pydanticIn,
    current_user: user_pydanticOut = Depends(
        get_admin_user
    ),
):
    ...
    return await category_pydantic.from_queryset_single(
        Category.get(id=category_obj.id)
    )


@router.post(
    "/categories/{id}/image",
    response_model=category_pydantic,
)
async def add_category_image(
    ...
    category_info = (
        await category_pydanticIn.from_queryset_single(
            Category.get(id=id)
        )
    )
    category_info.image_url = url
    await Category.filter(id=id).update(
...
async def update_category(
    id: int,
    request: category_pydanticUpdate,
    current_user: user_pydanticOut = Depends(
        get_admin_user
    ),
):
...
@router.post(
    "/products/add", response_model=product_pydantic
)
async def add_products(
    category_id: int,
    request: product_pydanticIn,
    current_user: product_pydantic = Depends(
        get_admin_user
    ),
):
        ...
        "data": json.dumps(
            {
                ...
                "screen_params": {
                    "name": f"{product_obj.name}",
                    "slug": f"{product_obj.slug}",
                },
            }
        ),
    }
    ...
    return await product_pydantic.from_queryset_single(
        Product.get(id=product_obj.id)
    )


@router.post(
    "/products/{id}/image", response_model=product_pydantic
)
async def add_product_image(
    ...
    url = await save_file(file)
    product_info = (
        await product_pydanticIn.from_queryset_single(
            Product.get(id=id)
        )
    )
    product_info.image_url = url
    await Product.filter(id=id).update(
        **product_info.dict(exclude_unset=True)
    )
    return await product_pydantic.from_queryset_single(
        Product.get(id=id)
    )
...
@router.put(
    "/products/{id}/update", response_model=product_pydantic
)
async def update_product(
    id: int,
    request: product_pydanticUpdate,
    current_user: user_pydanticOut = Depends(
        get_admin_user
    ),
):
...
@router.post(
    ...
    response_model=store_settings_pydantic,
)
async def update_store_settings(
    settings: store_settings_pydanticIn,
    current_user: user_pydanticOut = Depends(
        get_admin_user
    ),
):

    settings_obj = await StoreSettings.first().update(
        **settings.dict(exclude_unset=True)
    )
    return (
        await store_settings_pydantic.from_queryset_single(
            StoreSettings.first()
        )
    )
