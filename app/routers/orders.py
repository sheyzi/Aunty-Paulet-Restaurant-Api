...
async def order_product(
    order_details: order_pydanticIn,
    ...
    if order_details.amount > current_user.balance:
        raise HTTPException(
            ...
            detail="Insufficient balance!! Please fund your account to complete your order!!",
        )
    ...
    order_details["amount"] = sum + int(delivery_fee)
    ...
    user_info = await user_pydanticIn.from_queryset_single(
        User.get(id=current_user.id)
    )
    user_info.balance = float(user_info.balance) - float(
    ...
    data = {
        ...
        "body": "Your order of ₦{:.2f} was Successful. You would be contacted shortly".format(
            order_obj.amount
        ),
    ...
    admin_push_tokens = (
        await admin_push_token.from_queryset(
            AdminPushToken.all()
        )
    )
    ...
    await send_push_notification(
        push_tokens,
        "New Order",
        "There is a new food order in Aunty Paulet Restaurant!!",
    )
    await User.filter(id=current_user.id).update(
        **user_info.dict(exclude_unset=True)
    )
    return await order_pydantic.from_queryset_single(
        Order.get(id=order_obj.id)
    )
