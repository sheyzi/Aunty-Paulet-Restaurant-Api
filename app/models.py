...
class Category(Model):
    ...
    slug = fields.CharField(max_length=50, unique=True, null=False)
    image_url = fields.CharField(
        max_length=500, null=False, default='https://res.cloudinary.com/sheyzisilver/image/upload/v1628729409/placeholder_rl2wsr.png')
...
class UserPushToken(Model):
    id = fields.IntField(pk=True, index=True)
    user = fields.OneToOneField('models.User', related_name='token', on_delete=fields.CASCADE)
...
class Product(Model):
    ...
    slug = fields.CharField(max_length=60, unique=True, null=False)
...
class Order(Model):
    ...
    receiver_city = fields.CharField(max_length=20, default="Ikorodu")
    receiver_state = fields.CharField(max_length=20, default="Lagos")
    amount = fields.FloatField()
    payment_ref_id = fields.CharField(max_length=150)
    created_at = fields.DatetimeField(auto_now_add=True)
    status = fields.CharField(max_length=30, default='PENDING_PAYMENT')
...
class OrderItem(Model):
    ...
    product = fields.ForeignKeyField(
        "models.Product", related_name="items", on_delete=fields.RESTRICT)
...
class Transactions(Model):
    ...
    user = fields.ForeignKeyField("models.User", related_name="transactions", on_delete=fields.RESTRICT)



user_pydantic = pydantic_model_creator(User, name="User")
user_pydanticIn = pydantic_model_creator(
    User, name="UserIn", exclude_readonly=True, exclude=("is_active", "is_admin"))
user_pydanticUpdate = pydantic_model_creator(
    User, name="UserUpdate", exclude_readonly=True, exclude=("is_active", "is_admin", "password", "balance"))
...
category_pydanticUpdate = pydantic_model_creator(
    Category, name="CategoryUpdate", exclude_readonly=True, exclude=('image_url',))
...
product_pydanticUpdate = pydantic_model_creator(
    Product, name="ProductUpdate", exclude_readonly=True, exclude=('image_url',))

order_pydantic = pydantic_model_creator(Order, name="Order")
order_pydanticIn = pydantic_model_creator(
    Order, name="OrderIn", exclude_readonly=True,  exclude=('payment_ref_id',))
